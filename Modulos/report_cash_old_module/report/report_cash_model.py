...
class ReportCashModel(models.AbstractModel):
    _name = 'report.report_cash.report_invoice'
    ...
    def _sum_credit(self, data, journal_id, ):
        ...
        # print "***esto es journal", journal_id
        move_state = ['draft', 'posted']
        if data['form'].get('target_move', 'all') == 'posted':
            move_state = ['posted']

        query_get_clause = self._get_query_get_clause(data)
        # print "******esto es la query", query_get_clause
        params = [tuple(move_state), tuple(journal_id.ids)] + query_get_clause[2]
        # print "******esto es params", params
    ...
    def render_html(self, docids, data=None):
        ...
        # print "Estos son los docs .......",  self.env['account.journal'].browse(data['form']['journal_ids'])
        ...
        return self.env['report'].render('report_cash.report_invoice', docargs)